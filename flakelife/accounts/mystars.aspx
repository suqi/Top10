<%@ page title="" language="C#" masterpagefile="~/Site.master" autoeventwireup="true" inherits="qa_mystars, App_Web_nasjltov" %>
...
<title>我的收藏-<%=string.IsNullOrEmpty(Request.QueryString["tag"])?"":("关于"+Request.QueryString["tag"]+"的问题") %>-<%=ConfigHelper.SiteName %></title>
<style>
...
.ignore a{margin-right:20px;}
.unstar{margin-left:20px;}
</style>
</asp:Content>
...
<div class="mright" title="按标签在我的所有收藏中搜索">
    <form action="mystars.aspx" method="get" name="ssform">
...
<%
    ...
    var questions = BLL.Question.GetMyStarQuestions(CurrentUser["_id"].AsObjectId, Request.QueryString["tag"], pageNo, 15, out pagecount, out totalCount);
     %>
<div class="q-head">
    <div id="switcher"><h1>我的帐号</h1></div>
    <div id="tabs">
        <a href="profile.aspx">个人资料</a>
        <a href="my.aspx?p=<%=pageNo %><%=qtag %>">我的提问</a>
        <a href="myanswers.aspx?p=<%=pageNo %>">我的回答</a>
    	<a href="mystars.aspx?p=<%=pageNo %><%=qtag %>" class="current">我的收藏</a>
    </div>
</div>
        ...
        <td class="title">
            ...
            <div class="tags">
                <%foreach(var tag in question["tags"].AsBsonArray){ %>
                    <a href="mystars.aspx?tag=<%=Server.UrlEncode(tag.AsString) %>"><%=tag.AsString %></a>
                <%} %>
            </div>
            <div class="ignore">
...
<script>
    function unstar(a,qid){
        $.post('mystars.aspx',{'action':'unstar','questionid':qid},function(r){
            if(r&&r.status==200){
                $(a).parents('tr').fadeOut(1000).remove();
    ...
    $(function () {
        $('#q-more').click(function () {
            var more=this;
            $('#q-loading').show();
            $.post('loadmystars.aspx?p='+(index+1)+'<%=qtag %>',function(r){
