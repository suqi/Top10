<%@ page title="" language="C#" masterpagefile="~/Site.master" autoeventwireup="true" inherits="qa_solved, App_Web_wdpwwatm" %>
...
<div class="mleft">
    ...
    <a href="solved.aspx" class="youarehere">已解决</a>
    <a href="ask.aspx" id="nav-askquestion">提问</a>
</div>
<div class="mright" title="按标签在已经解决的问题中搜索">
    <form action="solved.aspx" method="get" name="ssform">
        <input type="text" maxlength="30" name="tag" class="s-text" value="<%=Request.QueryString["tag"] %>"  placeholder="按标签在已经解决的问题中搜索" />
...
<%
    ...
    QList.PageName = "solved";
    MyTags.UserItem = CurrentUser;    
    %>
<div style="clear:both;height:5px;"></div>
<div class="q-head">
    <div id="switcher"><h1><%=string.IsNullOrEmpty(Request.QueryString["tag"])?"已解决问题":("标签：<a title='查看此标签的简介以及用户排行' target='_blank' href='tag.aspx?key="+Server.UrlEncode(Request.QueryString["tag"])+"'>"+Request.QueryString["tag"]+"</a>") %></h1></div>
    <div id="tabs">
        <a href="solved.aspx?sort=newest&p=<%=pageNo %>" <%=(string.IsNullOrEmpty(Request.QueryString["sort"])||string.Compare(Request.QueryString["sort"],"newest",true)==0)?"class='current'":"" %> title="按提问时间先后排序">最新提问</a>
        <a href="solved.aspx?sort=latestreply&p=<%=pageNo %>" <%=string.Compare(Request.QueryString["sort"],"latestreply",true)==0?"class='current'":"" %> title="按最后回复时间排序">最新回答</a>
    	<a href="solved.aspx?sort=votes&p=<%=pageNo %>" <%=string.Compare(Request.QueryString["sort"],"votes",true)==0?"class='current'":"" %> title="看那些投票最多的问题">最多投票</a>
    	<a href="solved.aspx?sort=stars&p=<%=pageNo %>" <%=string.Compare(Request.QueryString["sort"],"stars",true)==0?"class='current'":"" %> title="看看大家都关注什么问题">最多收藏</a>
    </div>
</div>
...
<script>
    ...
    $(function () {
        $('#q-more').click(function () {
            var more=this;
            $('#q-loading').show();
            $.get('loadsolved.aspx?sort=<%=Request.QueryString["sort"] %><%=qtag %>&p='+(index+1),function(r){
