<%@ page title="" language="C#" masterpagefile="~/Site.master" autoeventwireup="true" inherits="qa_unsolved, App_Web_wdpwwatm" %>
...
<title>未解决的问题-<%=ConfigHelper.SiteName %></title>
<style>
...
<div class="mleft">
    ...
    <a href="unsolved.aspx"  class="youarehere">未解决</a>
    <a href="solved.aspx">已解决</a>
    <a href="ask.aspx"  id="nav-askquestion">提问</a>
</div>
<div class="mright" title="按标签在未解决的问题中搜索">
    <form action="unsolved.aspx" method="get" name="ssform">
        <input type="text" maxlength="30" name="tag" class="s-text" value="<%=Request.QueryString["tag"] %>"  placeholder="按标签在未解决的问题中搜索" />
...
<%
    ...
    var questions = BLL.Question.GetUnSolvedQuestions(Request.QueryString["sort"],Server.UrlDecode(Request.QueryString["tag"]),pageNo, 15, out pagecount,out totalCount);
    QList.Questions = questions;
    QList.PageName = "unsolved";
    MyTags.UserItem = CurrentUser;    
    %>
<div style="clear:both;height:5px;"></div>
<div class="q-head">
    <div id="switcher"><h1><%=string.IsNullOrEmpty(Request.QueryString["tag"])?"未解决问题":("标签：<a title='查看此标签的简介以及用户排行' target='_blank' href='tag.aspx?key="+Server.UrlEncode(Request.QueryString["tag"])+"'>"+Request.QueryString["tag"]+"</a>") %></h1></div>
    <div id="tabs">
        <a href="unsolved.aspx?sort=newest&p=<%=pageNo %>" <%=(string.IsNullOrEmpty(Request.QueryString["sort"])||string.Compare(Request.QueryString["sort"],"newest",true)==0)?"class='current'":"" %> title="按提问时间先后排序">最新提问</a>
        <a href="unsolved.aspx?sort=latestreply&p=<%=pageNo %>" <%=string.Compare(Request.QueryString["sort"],"latestreply",true)==0?"class='current'":"" %> title="按最后回复时间排序">最新回答</a>
    	<a href="unsolved.aspx?sort=votes&p=<%=pageNo %>" <%=string.Compare(Request.QueryString["sort"],"votes",true)==0?"class='current'":"" %> title="看那些投票最多的问题">最多投票</a>
    	<a href="unsolved.aspx?sort=stars&p=<%=pageNo %>" <%=string.Compare(Request.QueryString["sort"],"stars",true)==0?"class='current'":"" %> title="看看大家都关注什么问题">最多收藏</a>
        <a href="unsolved.aspx?sort=unanswered&p=<%=pageNo %>" <%=string.Compare(Request.QueryString["sort"],"unanswered",true)==0?"class='current'":"" %> title="救救那些迷茫中的孩子吧！" >零回复</a>
    </div>
</div>
...
<script>
    ...
    $(function () {
        $('#q-more').click(function () {
            var more=this;
            $('#q-loading').show();
            $.get('loadunsolved.aspx?sort=<%=Request.QueryString["sort"] %><%=qtag %>&p='+(index+1),function(r){
